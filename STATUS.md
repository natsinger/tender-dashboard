...
3e. Run `scripts/sql/enable_rls_all_tables.sql` in Supabase SQL Editor (enables RLS on all tables, revokes anon writes, adds user-scoped policies)
...
10. **RLS migration pending** — Run `scripts/sql/enable_rls_all_tables.sql` in Supabase SQL Editor to enable Row Level Security on all tables. Without this, the anon key (visible in browser) allows unrestricted read/write.
