...
| Date | Change | Files |
|------|--------|-------|
| 2026-03-06 | **Fix mitcham/gush data pipeline (Issue #12)** — Root cause: API `MitchamName` was cast to INT for `lot_number`, losing non-numeric values like "2 א" or compound IDs. Fix: (1) **data_client.py** — `extract_lots_from_api()` now preserves raw MitchamName in `mitcham_name` TEXT field; non-numeric values use 1-based positional fallback for `lot_number`; `gush_helka_raw` JSONB stores structured GushHelka[] array. (2) **db.py** — `upsert_lots()` uses delete-then-insert pattern (partial unique indexes per data_source make single ON CONFLICT impossible); `mitcham_name` and `gush_helka_raw` included in DB row. (3) **scripts/extract_lots_batch.py** — merge logic preserves `mitcham_name` from API in all strategies; PDF `lot_number` is canonical in merged records; `_enrich_pdf_with_api_aggregate()` propagates mitcham_name + gush_helka_raw. (4) **SQL migration** (`scripts/sql/fix_mitcham_gush_schema.sql`) — adds `mitcham_name` TEXT + `gush_helka_raw` JSONB columns, backfills mitcham_name from existing lot_number, replaces UNIQUE constraint with partial unique indexes per data_source, all idempotent. (5) **frontend/src/types/database.ts** — `mitcham_name` and `gush_helka_raw` added to TenderLot interface. (6) **frontend/src/components/explorer/detail-viewer.tsx** — lots table shows "מזהה רמ״י" column for API mitcham_name (conditional); API section labels lots as "מזהה רמ״י" not "מתחם". (7) **tests/test_mitcham_gush_fix.py** — 28 new tests covering: mitcham_name preservation, positional fallback, gush_helka_raw, merge strategies, edge cases. 329 tests pass, Next.js build clean. Only Next.js frontend updated (Streamlit is legacy/deprecated). | `data_client.py`, `db.py`, `scripts/extract_lots_batch.py`, `scripts/sql/fix_mitcham_gush_schema.sql` (NEW), `frontend/src/types/database.ts`, `frontend/src/components/explorer/detail-viewer.tsx`, `tests/test_mitcham_gush_fix.py` (NEW) |
...
~~12. **MitchamName mislabeled as מתחם** — **RESOLVED** (2026-03-06). MitchamName is now stored in dedicated `mitcham_name` TEXT field, PDF sequential numbers used for `lot_number`, API label changed to "מזהה רמ״י". See `fix_mitcham_gush_schema.sql` migration. Run migration in Supabase SQL Editor before deploying.~~
...
│   └── test_mitcham_gush_fix.py    # Tests for mitcham/gush fix (28 tests)
...
│       └── fix_mitcham_gush_schema.sql  # SQL: mitcham_name + gush_helka_raw columns + partial unique indexes
